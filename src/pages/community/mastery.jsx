...
import { AlertCircle, Trophy, Target, TrendingUp } from 'lucide-react';

const CommunityMastery = () => {
  ...
  useEffect(() => {
    const leaderboardRef = query(
      ref(database, 'users'),
      orderByChild('stats/mastery/rating'),
    );
    ...
    const unsubscribe = onValue(leaderboardRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        snapshot.forEach((child) => {
          ...
          const stats = data.stats?.mastery;
    ...
    <div className="min-h-screen bg-[#F0EFEB] text-[#1a1a1a] font-sans">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-500/10 text-gray-600 mb-4">
            <AlertCircle size={16} />
            <span className="text-sm font-medium">Mastery Leaderboard</span>
          </div>
          <h1 className="text-4xl font-bold tracking-tight mb-3">Community Rankings</h1>
          <p className="text-gray-500 max-w-md mx-auto">
            Track your progress mastering the most challenging verb conjugations
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-[#DEDDDA] p-12 text-center">
            ...
            <p className="text-gray-500">Be the first to play Mastery and claim the top spot!</p>
          </div>
        ) : (
          <>
...
export default CommunityMastery;
